...
@section('main-content')
	<div class="container-fluid spark-screen">
        <div class="box box-primary">
            ...
            <!-- /.box-header -->
            <!-- form start -->
            <form role="form" method="post" action="{{ route('updateNew',['new' => $slider->id]) }}" enctype="multipart/form-data">
                <input type="hidden" name="_token" value="{{{ csrf_token() }}}" />
                <div class="box-body">
                    <div class="col-sm-12">
                        <div class="col-sm-6">
                        <label for="exampleInputkey">Titulo</label>
                            <input type="text" name="title" class="form-control" id="exampleInputkey" value="{{ $slider->title }}">
                        </div>
                        <div class="col-sm-6">
                            <label for="exampleInputkey">Subtitulo</label>
                            <textarea name="short_description" class="form-control" rows="3" placeholder="Enter ...">{{ $slider->short_description }}</textarea>
                        </div>
                    </div>

                    <div class="col-sm-12">
                        <label>Descripcion</label>
                        <textarea name="description" id="editor1" class="form-control ckeditor" rows="8" placeholder="Enter ...">{{ $slider->description }}</textarea>
                    </div>

                    <div class="col-sm-6">
                        <label>Video</label>
                        <input type="text" name="video" class="form-control" id="exampleInputkey" value="{{ $slider->video }}">
                ...
                <div class="box-body">
                    ...
                    <div class="form-group">
                        ...
                        <div class="timeline-body" style="background-color: #545257; width: 220px;">
                            @foreach($slider->attachments as $attachment)
                                @if($attachment->id == $slider->img_principal)
                    ...
                    <div class="form-group">
                        ...
                        <div class="timeline-body" style="background-color: #545257;">
                            @foreach($slider->attachments as $attachment)
                                @if($attachment->id != $slider->img_principal && $attachment->id != $slider->img_thumbnail)
